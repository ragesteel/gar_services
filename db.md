...
### 2026-03-12 08:40 MSK

Однако определённо стоит все метаданные по эелементам GarRecord свести в отдельный record, чтобы не думать постоянно, а из какого пакета там нужно взять `@Nullable`…

### 2026-03-12 18:03 MSK

Упёрся-таки в проблему с H2/PostgreSQL. H2 сохраняет правильную дату, если дата отстоит больше чем на 100 лет от текущего времени.
Говоришь ему — сохранить 1900-01-01, а он в ответ выдаёт 1899-12-31. Но это я с датами через setDate/getDate работаю.
Да, лучше-бы тут использовать LocalDate, но вот PostgreSQL этого не понимает.

Всё-таки пожалуй стоит избавится от java.sql.Date и вернуть всё на LocalDate, разве что без указания на конкретный тип JDBCType или как там его.

### 2026-03-13 08:19 MSK

Пора-бы уже задачку #9 закрывать. По сути осталось только разобрать оставшиеся проблемы хранением данных в H2. Хочется чего-нибудь такого, чтобы работало и в PostgreSQL и в H2.
Видимо просто нужно оставить для LocalDate setObject всё!
